...
router.get("/", async (req, res, next) => {
  try {
    const { search, location, category, dateFound, userId, status, page = 1, limit = 12 } = req.query;
...
router.post("/", authenticate, upload.single("image"), async (req, res, next) => {
  try {
    const { name, location, description, dateFound, category, status } = req.body;
    ...
    try {
      ...
      if (users.length > 0) {
        ...
        console.log(`Created ${notifications.length} notifications for new item: ${itemId}`);
      }
...
router.put("/:id", authenticate, async (req, res, next) => {
  try {
    ...
    if (isClaimingItem && !isOwner) {
      if (name || location || description || dateFound || category || image !== undefined) {
        return res.status(403).json({ message: "You can only claim items. Only the owner can update other fields." });
      }
    }
    ...
    if (isClaimingItem && item.userId !== req.userId) {
      ...
      try {
        ...
        console.log(`Created claim notification for item owner: ${item.userId}, item: ${id}`);
      } catch (notificationError) {
